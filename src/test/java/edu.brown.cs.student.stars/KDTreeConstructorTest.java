...
public class KDTreeConstructorTest {
  ...
  public void testBuildTree() {
    ...
    // tests 2D coordinates
    test1C = new ArrayList<>();
    test1C.add(0.0);
    test1C.add(0.0);
    test1 = new Star(0, "test1", test1C);

    test2C = new ArrayList<>();
    test2C.add(1.0);
    test2C.add(0.0);
    test2 = new Star(1, "test2", test2C);

    test3C = new ArrayList<>();
    test3C.add(-1.0);
    test3C.add(2.0);
    test3 = new Star(2, "test3", test3C);

    test4C = new ArrayList<>();
    test4C.add(5.0);
    test4C.add(0.20);
    test4 = new Star(3, "test4", test4C);

    builder = new KDTreeConstructor<>();
    stars = new ArrayList<>();
    stars.add(test1);
    tree1 = builder.buildTree(stars, 1);
    assertEquals(tree1.getVal(), test1);
    assertEquals(tree1.getDepth(), 1);
    assertNull(tree1.getLeft());
    assertNull(tree1.getRight());
    ...
    stars.add(test3);
    stars.add(test4);
    tree1 = builder.buildTree(stars, 0);
    assertEquals(tree1.getVal(), test2);
    assertEquals(tree1.getDepth(), 0);
    assertEquals(tree1.getLeft().getVal(), test3);
    assertEquals(tree1.getRight().getVal(), test4);
    assertEquals(tree1.getLeft().getLeft().getVal(), test1);
